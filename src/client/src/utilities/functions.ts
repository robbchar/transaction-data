import { MonthYearDisabledType, transaction } from '../types/DataType';

export const organizeTheData = (
  transactions: transaction[],
): Map<number, Map<number, transaction[]>> => {
...
export const getDataToView = (
  ...
  organizedData.forEach(
    (
      ...
      year: number,
      yearMap: Map<number, Map<number, transaction[]>>,
    ) => {
      monthMap.forEach(
        (
          transactions: transaction[],
          month: number,
          map: Map<number, transaction[]>,
        ) => {
...
export const returnMonthYearKey = (month: number, year: number) =>
  `${month}_${year}`;
